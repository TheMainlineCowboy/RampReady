...
5a. PARTIAL / MOTION + STATIC STRUCTURE LOCKED — A1 push-sequence departure now animates correctly and advances the trainer. **DO NOT alter the current moving-bridge kinematics or the restored terminal-side static sections without explicit user direction.** User visually confirmed the aircraft-side moving bridge retracts correctly, stays physically attached at its terminal-side elevated hinge, the wheel/support carriage remains on the pavement, and the bridge height changes by tilting about the fixed hinge rather than translating the whole assembly. User also visually confirmed the two previously missing static terminal-side jetway sections are now present and correctly fixed in place. Their source/runtime authority is the exact XP11 `Jetway_1_solid.fac` Segment 4/5/6 shell geometry restored only on A1's four fixed `Rotunda_extension` walls; the thin control/end planes remain hidden. The deployed live verification for `56fa2a2550034381a618af5e41ef68c1b2f590a8` passed attached → midpoint → parked with 0.000000 m terminal-hinge gap, 0.000000 m support-bottom drift, 0.000000 m cabin-joint gap, and 0.000000 m fixed-wall movement. **LOCKED: moving bridge + fixed terminal-side sections.** OPEN VISUAL ITEM: later correct the exterior stair articulation on the moving bridge so the stair foot remains on/above pavement while stair angle changes with bridge height—flatter when the aircraft door is low, progressively steeper as the bridge rises/retracts. Do not regress the locked hinge/yaw/telescope/pitch/wheel behavior or the restored static sections while fixing the stairs.
