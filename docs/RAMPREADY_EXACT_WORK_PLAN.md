...
5. ✅ COMPLETE — Verify/fix LEKTRO driving controls and steering in the live app. User-confirmed FWD/REV and steering behavior remain correct; locked 15-ft turning radius, 9 mph empty / 4 mph towing, rear-wheel steering and existing physics are preserved. The finalized R187A authored rear steering pivots `AP88_STEER_L_STEER` / `AP88_STEER_R_STEER` now visibly articulate with the validated steering angle (live test: 0° → -84° → 0°). Operator View was also corrected from the passenger seat to the actual R187A driver position after the required 180° model-forward correction; live evidence reports local eye `[0.45, 1.35, -2.15]` and zero camera-to-eye offset.
5a. Fix the push-sequence jetway departure blocker. The training flow currently reaches the jetway-departure step and cannot advance because the exact live jetway is not yet performing the required departure animation/state transition. Implement and verify the exact sequence against the authored jetway geometry: hood clear, telescope/retract, rotate away/clear the aircraft, then signal the trainer state machine to advance. Do not fake the sequence with a timer-only bypass; the visual jetway state and training progression must agree.
