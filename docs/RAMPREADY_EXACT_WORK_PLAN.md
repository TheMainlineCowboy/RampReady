...
5. PARTIAL — LEKTRO driving controls and rear-wheel visual steering are verified/fixed. User-confirmed FWD/REV and steering behavior remain correct; locked 15-ft turning radius, 9 mph empty / 4 mph towing, rear-wheel steering and existing physics are preserved. The finalized R187A authored rear steering pivots `AP88_STEER_L_STEER` / `AP88_STEER_R_STEER` now visibly articulate with the validated steering angle (live test: 0° → -84° → 0°). OPEN / NEXT: fix Operator View so the camera is in the actual driver station rather than the passenger seat, and remove the current per-frame look-direction lock so touch/mouse look-around and the existing gyro pointer input work while in Operator View. Preserve the verified vehicle physics/controls. User visual verification of the rendered driver/steering-wheel side is required before marking item 5 complete.
