...
const camera=new THREE.PerspectiveCamera(34,1200/800,0.05,200);
...
const floor=new THREE.Mesh(new THREE.PlaneGeometry(60,60),new THREE.MeshStandardMaterial({color:0xd7dce1,roughness:0.94,metalness:0}));
floor.rotation.x=-Math.PI/2; floor.position.y=-0.02; floor.receiveShadow=true; scene.add(floor);
const grid=new THREE.GridHelper(60,30,0xb8bec5,0xcbd0d5); grid.position.y=-0.01; grid.material.transparent=true; grid.material.opacity=.30; scene.add(grid);
const views={
...
const selected=views[label]||views['front-left']; camera.position.fromArray(selected.p); camera.lookAt(...selected.t);
document.getElementById('status').textContent=label.replaceAll('-',' ');
const loader=new GLTFLoader();
loader.load('/${modelRelative}',gltf=>{
 const model=gltf.scene; scene.add(model);
 const livery=[]; const materials=[];
 model.traverse(o=>{
  ...
  for(const m of ms){
   ...
   if(/American_Eagle_Title|Registration|Tail_Livery/.test(o.name)){
     m.alphaTest=Math.max(m.alphaTest||0,0.015); m.transparent=false; m.depthWrite=true; m.side=THREE.DoubleSide; m.needsUpdate=true;
   }
   materials.push({object:o.name,material:m.name||'',hasMap:!!m.map,transparent:!!m.transparent,alphaTest:m.alphaTest||0});
  }
  if(/American_Eagle_Title|Registration|Tail_Livery/.test(o.name)) livery.push(o.name);
 });
 const box=new THREE.Box3().setFromObject(model); const size=box.getSize(new THREE.Vector3());
 window.__QA_REPORT__={view:label,liveryNodes:livery,materials:materials.filter(x=>/American_Eagle_Title|Registration|Tail_Livery/.test(x.object)),dimensions:{x:size.x,y:size.y,z:size.z}};
 ...
 window.__QA_REPORT__.pixelEvidence={red,blue,dark};
...
const server=http.createServer((req,res)=>{
  ...
  if(url.pathname==='/qa.html'){res.writeHead(200,{'Content-Type':'text/html'});res.end(html);return;}
  const clean=decodeURIComponent(url.pathname).replace(/^\/+/, '');
  const target=path.resolve(repoRoot, clean);
  if(!target.startsWith(repoRoot)||!fs.existsSync(target)||fs.statSync(target).isDirectory()){res.writeHead(404);res.end('not found');return;}
...
const viewNames=['front-left','front-right','left-side','right-side','rear','top-oblique','ramp-close'];
const reports=[];
...
const failures=[];
for(const required of ['American_Eagle_Title_Right','American_Eagle_Title_Left','Registration_Right','Registration_Left','Tail_Livery_Right','Tail_Livery_Left']) if(!liveryNames.has(required)) failures.push(`missing ${required}`);
if(mapped<6) failures.push(`only ${mapped}/6 livery materials have texture maps`);
if(maxRed<250||maxBlue<250) failures.push(`insufficient visible livery color evidence red=${maxRed} blue=${maxBlue}`);
...
console.log(`Three.js livery QA passed. Visible pixels red=${maxRed}, blue=${maxBlue}; mapped livery materials=${mapped}/6`);
