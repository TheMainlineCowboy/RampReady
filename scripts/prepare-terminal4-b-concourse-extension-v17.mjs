...
const surfaceImport = 'import { installTerminal4BConcourseSurfaceV21 } from "./terminal4BConcourseSurfaceV21.js";';
...
if (!source.includes(surfaceImport)) {
  if (!source.includes(detailImport)) throw new Error(`${authoredPath}: missing B-concourse detail import for surface pass`);
  source = source.replace(detailImport, `${detailImport}\n${surfaceImport}`);
}
...
const surfaceCall = `  const terminal4BConcourseSurface = installTerminal4BConcourseSurfaceV21(terminal4BConcourseExtension, {
    mainXMin: bMainXMin,
    mainXMax: bMainXMax,
    mainZMin: bMainZMin,
    mainZMax: bMainZMax,
...
if (!source.includes("installTerminal4BConcourseSurfaceV21(terminal4BConcourseExtension")) {
  if (!source.includes(detailCall)) throw new Error(`${authoredPath}: missing B-concourse detail call for surface pass`);
...
const sourceEvidence = `  sourcePlacedJetways.userData.terminal4BConcourseExtensionAuthority = terminal4BConcourseExtension.userData.authority;
  ...
  sourcePlacedJetways.userData.terminal4BConcourseSurfaceAuthority = terminal4BConcourseSurface.userData.authority;
...
if (!source.includes("terminal4BConcourseExtensionAuthority = terminal4BConcourseExtension.userData.authority")) {
  if (!source.includes(surfaceCall)) throw new Error(`${authoredPath}: missing B-concourse surface call for evidence`);
  source = source.replace(surfaceCall, `${surfaceCall}\n${sourceEvidence}`);
} else {
  if (!source.includes("terminal4BConcourseDetailAuthority")) {
  ...
  if (!source.includes("terminal4BConcourseSurfaceAuthority")) {
    const evidenceAnchor = "  sourcePlacedJetways.userData.terminal4BConcourseDetailAuthority = terminal4BConcourseDetail.userData.authority;";
    source = source.replace(evidenceAnchor, `${evidenceAnchor}\n  sourcePlacedJetways.userData.terminal4BConcourseSurfaceAuthority = terminal4BConcourseSurface.userData.authority;`);
  }
}
...
const environmentEvidence = `  environment.userData.authoredTerminal4BConcourseExtensionAuthority = sourcePlacedJetways.userData.terminal4BConcourseExtensionAuthority;
  ...
  environment.userData.authoredTerminal4BConcourseSurfaceAuthority = sourcePlacedJetways.userData.terminal4BConcourseSurfaceAuthority;
...
if (!source.includes("authoredTerminal4BConcourseExtensionAuthority")) {
  ...
  if (!source.includes("authoredTerminal4BConcourseDetailAuthority")) {
  ...
  if (!source.includes("authoredTerminal4BConcourseSurfaceAuthority")) {
    const evidenceAnchor = "  environment.userData.authoredTerminal4BConcourseDetailAuthority = sourcePlacedJetways.userData.terminal4BConcourseDetailAuthority;";
    source = source.replace(evidenceAnchor, `${evidenceAnchor}\n  environment.userData.authoredTerminal4BConcourseSurfaceAuthority = sourcePlacedJetways.userData.terminal4BConcourseSurfaceAuthority;`);
  }
}
...
for (const token of [
  ...
  "installTerminal4BConcourseSurfaceV21(terminal4BConcourseExtension",
  "terminal4BConcourseExtensionAuthority",
  "terminal4BConcourseDetailAuthority",
  "terminal4BConcourseSurfaceAuthority",
  ...
  "authoredTerminal4BConcourseSurfaceAuthority",
...
console.log("Prepared the missing Terminal 4 B-concourse from the supplied PHX aerial and B15-B28 source gate coordinates, then added simulator-detail panel joints, framed B15 portals, weathered facade texture maps, readable gate signs, ramp utilities, parapets and irregular rooftop equipment.");
