...
// The final-world camera already resolves the exact visible Tunnel_C hierarchy,
// scans its transformed vertices, isolates the lowest 10 cm contact band, proves
// a multi-point support footprint, checks that footprint is aircraft-side along
// the Rotunda->Cab bridge, and requires it to be on the ramp. The retired 4 m
// centerline guard is not a valid bogie identity test: the authored Tunnel_C
// support truck can be laterally offset from the Rotunda/Cab centerline while
// still being the real supplied wheel/support geometry. Keep lateral offset as
// diagnostic telemetry, but validate identity from the exact Tunnel_C footprint.
...
for (const required of [
  ...
  if (!source.includes(required)) {
    throw new Error(`${trainerPath}: final-world Tunnel_C footprint camera is missing ${required}`);
  }
}
for (const forbidden of [
  ...
  if (source.includes(forbidden)) {
    throw new Error(`${trainerPath}: obsolete Tunnel_C centerline bogie rejection remains: ${forbidden}`);
  }
}

fs.writeFileSync(trainerPath, source, "utf8");
console.log(`Prepared ${marker}: final bogie evidence targets the transformed visible Tunnel_C low-contact footprint, retains aircraft-side ordering and ramp-contact checks, and keeps lateral offset as diagnostics without treating the Rotunda/Cab centerline as wheel geometry.`);
