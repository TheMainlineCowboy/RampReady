...
for (const token of [
  ...
  if (!jetways.includes(token)) throw new Error(`AIR_Jetway01 CRJ-scale source is missing ${token}`);
}
...
for (const [oldToken, newToken] of [
  ["CRJ_FORWARD_DOOR_AFT_OF_NOSE_GEAR_METERS = 4.1", "CRJ_FORWARD_DOOR_AFT_OF_NOSE_GEAR_METERS = 5.35"],
  ["CRJ_FORWARD_DOOR_LEFT_OF_CENTERLINE_METERS = 1.55", "CRJ_FORWARD_DOOR_LEFT_OF_CENTERLINE_METERS = 1.35"],
  ["AIR_JETWAY01_CONTACT_CLEARANCE_METERS = 2.78", "AIR_JETWAY01_CONTACT_CLEARANCE_METERS = 1.65"],
]) {
...
for (const token of [
  ...
  if (!verifier.includes(token)) throw new Error(`AIR_Jetway01 verifier is missing CRJ-scale token ${token}`);
}
fs.writeFileSync(verifierPath, verifier, "utf8");
console.log("Prepared AIR_Jetway01 CRJ-scale v3 alignment: aft forward-door target, compact cabin, shorter bellows and realistic bridge clearance.");
