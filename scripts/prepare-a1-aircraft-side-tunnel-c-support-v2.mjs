...
// The Aug. 15 reference/photo normalizer is the final authority for the remote
// Rotunda and A1-only fixed corridor. Do not attempt to relocate the Rotunda here
// from a retired compact-reach code anchor: this stage runs after that normalizer
// and owns only aircraft-side Tunnel-C articulation/support selection.
console.log(`Kept A1 Tunnel_C longitudinally with the Cab and restricted grounding to measured aircraft-side support descendants (Rotunda-to-Cab ratio >= ${minimumAircraftSideRatio}); preserved the Aug. 15 source-registered remote Rotunda/fixed-corridor authority and exact supplied GLB vertices.`);
