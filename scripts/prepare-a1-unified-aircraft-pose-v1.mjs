...
// fleet. A1 is intentionally different: its Aug. 15 photo-authoritative terminal
// geometry is owned by the BGATE1 long fixed corridor/dogleg/remote-Rotunda
// stages. Do not re-run the retired compact-wall finalizer here; doing so
// re-imposes the old 1.2-3.6 m sleeve assumptions after the long route has been
// constructed and prevents the production bundle from reaching the browser.
...
console.log("Kept all 57 static Rotundas at their exact source poses with their real measured Terminal 4 fixed-leg spans, preserved the exact supplied jetway ground-contact offset on the complete 58-gate fleet, preserved A1's Aug. 15 BGATE1 long fixed corridor/dogleg/remote-Rotunda authority without re-running the retired compact-wall finalizer, then locked A1 to one physical aircraft pose across training and free-drive inspection.");
