...
for (const forbidden of [
  ...
  'groundedComponentCount !== 5',
  'groundedTriangleCount !== 1311',
]) {
  if (source.includes(forbidden)) throw new Error(`${trainerPath}: stale visible-support fixed-set authority survived: ${forbidden}`);
}

fs.writeFileSync(trainerPath, source, "utf8");
console.log(`Prepared ${marker}: complete above-pavement compact Tunnel-C hardware is grounded by rendered KPHX pavement raycasts and rescanned fail-closed.`);
