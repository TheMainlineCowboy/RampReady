...
const marker = "a1-visible-tunnel-c-support-grounding-runtime-v11-crossing-branch-scan";
...
for (const required of [
  ...
  importLine,
  authority,
  "rendered-kphx-source-aerial-raycast-under-wholly-suspended-and-crossing-hanging-branches",
  `finalA1VisibleSupportGrounding.inspectedCandidateCount !== ${EXPECTED_INSPECTED_COMPONENT_COUNT}`,
  `finalA1VisibleSupportGrounding.groundedComponentCount !== ${EXPECTED_WHOLE_SUSPENDED_COMPONENT_COUNT}`,
  `finalA1VisibleSupportGrounding.groundedTriangleCount !== ${EXPECTED_WHOLE_SUSPENDED_TRIANGLE_COUNT}`,
  `finalA1VisibleSupportGrounding.buriedCandidateCount !== ${EXPECTED_BURIED_COMPONENT_COUNT}`,
...
console.log(`Prepared ${marker}: all 25 compact Tunnel-C islands are classified, the five wholly suspended islands plus every hanging branch inside mixed crossing hardware are grounded against rendered KPHX pavement, and the final corrected set must have zero suspension.`);
