...
for (const required of [
  ...
  if (!source.includes(required)) {
    throw new Error(`${doorFitPath}: integrated Tunnel-C preservation is missing ${required}`);
  }
}

fs.writeFileSync(doorFitPath, source, "utf8");
console.log(`Prepared ${marker}: the exact supplied Tunnel-C passenger carrier can no longer be vertically translated as bogie/stair grounding; only genuinely separable small support meshes may move.`);
