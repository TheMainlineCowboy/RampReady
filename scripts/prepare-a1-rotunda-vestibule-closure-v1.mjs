...
// compact-reference ceiling. Make the already measured real terminal distance
// authoritative here, then remove the obsolete symbol entirely.
...
if (!installation.includes("const desiredTerminalDistance = terminalDistance;")) {
  throw new Error(`${installationPath}: final rigid-parent desired distance is not sourced from the real terminal distance`);
}
for (const forbidden of [
  ...
  if (installation.includes(forbidden)) {
    throw new Error(`${installationPath}: generated/compact Rotunda geometry survived final connector preparation: ${forbidden}`);
  }
}
...
console.log("Prepared A1 visual acceptance from the finalized source-measured Terminal 4 wall geometry: no fake Rotunda bulkhead, no executable 2.4 m relocation, exact supplied Rotunda preserved, and the rigid parent uses the measured real terminal distance.");
