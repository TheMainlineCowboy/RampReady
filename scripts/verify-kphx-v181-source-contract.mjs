...
for (const token of ["CanvasTexture", "InstancedMesh", "KPHX_Terminal4_JetwayBridges", "KPHX_Terminal4_GateLeadIns"]) {
  if (!files.builder.includes(token)) throw new Error(`KPHX builder contract missing ${token}`);
}
for (const token of ['groundSource = "authored-kphx-v181"', "sourceJetwayCount", "b15Anchors"]) {
  if (!files.ground.includes(token)) throw new Error(`KPHX runtime contract missing ${token}`);
}
for (const token of ['dataset.kphxVersion', 'dataset.sourceJetwayCount', 'dataset.b15Anchors']) {
  if (!files.prepare.includes(token)) throw new Error(`KPHX browser evidence missing ${token}`);
}
console.log(`Verified KPHX v1.8.1 source contract: ${parkingCount} Terminal 4 stands and ${jetwayCount} jetways.`);
