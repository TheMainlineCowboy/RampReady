...
const replacement = `canvas.evaluate((element, keys) => Object.fromEntries(keys.map((key) => [key, element.dataset[key]])), ${JSON.stringify(keys)})`;
...
console.log(`Bounded A1 terminal-joint dataset transfer to ${keys.length} fields across ${occurrences} reads.`);
