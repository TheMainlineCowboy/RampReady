...
if (!fleetSource.includes(authority)) {
  ...
  const genericGroundReference = "    -correctedSourceBounds.min.y,";
  const wheelGroundReference = "    -sourceWheelGroundY,";
  if (!fleetSource.includes(genericGroundReference)) throw new Error(`${fleetPath}: generic whole-model ground reference is missing`);
  fleetSource = fleetSource.replace(genericGroundReference, wheelGroundReference);

  const positionedAnchor = "  sourceScene.updateMatrixWorld(true);\n\n  const aligned = new THREE.Group();";
  const positionedCheck = `  sourceScene.updateMatrixWorld(true);\n  const groundedSourceWheelPair = measureExactSourceWheelPair(THREE, sourceScene);\n  const groundedWheelBottomDelta = Math.abs(groundedSourceWheelPair[0].minimumY - groundedSourceWheelPair[1].minimumY);\n  const maximumGroundedWheelBottom = Math.max(Math.abs(groundedSourceWheelPair[0].minimumY), Math.abs(groundedSourceWheelPair[1].minimumY));\n  if (groundedWheelBottomDelta > 0.01 || maximumGroundedWheelBottom > 0.01) {\n    throw new Error(\`Exact Airport_Jetway.glb paired wheels are not the normalized ground contact: bottoms=\${groundedSourceWheelPair.map((wheel) => wheel.minimumY).join(",")}\`);\n  }\n  let lowestSourceMeshName = "";\n  let lowestSourceMeshY = Number.POSITIVE_INFINITY;\n  sourceScene.traverse((object) => {\n    if (!object?.isMesh) return;\n    const box = new THREE.Box3().setFromObject(object);\n    if (!box.isEmpty() && box.min.y < lowestSourceMeshY) {\n      lowestSourceMeshY = box.min.y;\n      lowestSourceMeshName = object.name || "unnamed";\n    }\n  });\n  const sourceGroundPenetrationMeters = Math.max(0, -lowestSourceMeshY);\n  if (!Number.isFinite(sourceGroundPenetrationMeters) || sourceGroundPenetrationMeters > 0.75) {\n    throw new Error(\`Exact Airport_Jetway.glb has excessive geometry below the paired-wheel ramp plane: \${lowestSourceMeshName}=\${lowestSourceMeshY} m\`);\n  }\n  console.log(\`[exact-airport-jetway] paired wheels grounded; deepest source mesh \${lowestSourceMeshName}=\${lowestSourceMeshY.toFixed(4)} m below wheel plane\`);\n\n  const aligned = new THREE.Group();`;
...
for (const required of [
  ...
  "const sourceWheelGroundY = Math.min",
  "-sourceWheelGroundY",
  "groundedWheelBottomDelta > 0.01",
  "sourceGroundPenetrationMeters > 0.75",
...
if (fleetSource.includes("-correctedSourceBounds.min.y")) {
  throw new Error(`${fleetPath}: whole-model minimum is still being used as the exact jetway ramp ground reference`);
}
...
console.log("Prepared exact Airport_Jetway.glb as one rigid source assembly with its authored two-wheel axle level and the paired tire bottoms defining the ramp plane; removed the obsolete 6 cm fleet-wide vertical fudge.");
