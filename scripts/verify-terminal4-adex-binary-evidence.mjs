...
fixture.write('Terminal 4 gates A1 A10 B15 B15', 112, 'ascii');
fixture.write('NOTA100 XB15Z', 176, 'ascii');
...
assert.equal(summary.totalCandidateOccurrenceCount, 4);
assert.equal(summary.labels.find(({ label }) => label === 'B15').occurrenceCount, 2);
assert.deepEqual(summary.labels.find(({ label }) => label === 'B15').sourceByteOffsets, [136, 140]);
assert.equal(summary.requiredCoverage.find(({ label }) => label === 'B15').candidateLabelPresent, true);
assert.equal(summary.requiredCoverage.find(({ label }) => label === 'B14').candidateLabelPresent, false);
assert.equal(summary.allRequiredCandidateLabelsPresent, false);
assert(summary.interpretation.includes('does not prove a parking-record relationship'));
assert.deepEqual(REQUIRED_TERMINAL4_CORRIDOR_LABELS, [
