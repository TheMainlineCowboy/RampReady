...
if len(sys.argv) != 6:
    raise SystemExit("usage: stage-kphx-xp12-lib.py <xp12-root> <audit.json> <directory.txt> <out> <extensions-csv>")
...
# Pinned 12.1.2 manifest authority.
manifest={}
cur=""
...
while queue:
    ...
    if not authority:
        raise RuntimeError(f"file not in pinned 12.1.2 manifest: {rel}")
    actual=md5(p)
...
(out/"kphx-xp12-lib-chunk-manifest.json").write_text(json.dumps(report,indent=2)+"\n")
print(json.dumps({
    "requestedVirtualCount":len(requested),
    "resolvedVirtualCount":len(resolved),
    "copiedPhysicalFileCount":len(copied),
