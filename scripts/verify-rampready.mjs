...
for (const optionalCss of ["src/components/throttle-visibility.css", "src/components/throttle-force.css"]) {
  if (existsSync(optionalCss)) {
    ...
    if (optionalCss.includes("throttle-force")) {
      ...
      for (const marker of [".rr-throttle::before", ".rr-steer::before", "Power / Direction", "Steer / Brake"]) requireHard(css.includes(marker), `Touch cluster label missing expected marker: ${marker}`);
