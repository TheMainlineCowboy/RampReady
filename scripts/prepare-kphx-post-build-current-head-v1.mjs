...
// baselines after producing dist/. Reapply the current-head acceptance migration
// before Playwright inspects the exact production artifact. This script changes
// browser expectations only; it never changes runtime geometry or the supplied
// Airport_Jetway.glb.
...
// Keep the full-airport inspection route and CI-stable bidirectional motion gate.
{
...
// KPHX acceptance follows the airport-owned A1 architecture: the decoded PHX
// jetway/Rotunda remains fixed and the inspection aircraft is positioned so its
// forward-left door meets the final Cab. Do not force the airplane back to the
// retired raw (0, 6.2) stop and do not infer correctness from relocation deltas.
{
  ...
  if (source.includes(staleLaunchPoseBlock)) {
    source = source.replace(staleLaunchPoseBlock, cabDerivedLaunchPoseBlock);
  } else if (!source.includes("data?.inspectionAircraftFixedSourceGateAuthority === aircraftAuthority")) {
    throw new Error(`${path}: Cab-derived launch-pose acceptance anchor is missing`);
  }
  ...
  if (source.includes(staleRelocationAssertBlock)) {
    source = source.replace(staleRelocationAssertBlock, cabDerivedPoseAssertBlock);
  } else if (!source.includes("inspectionAircraftFixedSourceGateAuthority")) {
    throw new Error(`${path}: Cab-derived A1 pose assertion block is missing`);
  }
  ...
  if (source.includes(staleNoseGearBlock)) {
    source = source.replace(staleNoseGearBlock, cabDerivedNoseGearBlock);
  } else if (!source.includes("const noseGearX = Number(runtime.inspectionAircraftNoseGearX)")) {
    throw new Error(`${path}: Cab-derived A1 nose/door assertion is missing`);
  }

  // The main migration converts Cab-normal heading to authored PHX source
  // heading. Use the unified live pose field rather than compatibility yaw.
...
// The close A1 terminal/bogie evidence suite proves the same final architecture:
// Cab-derived aircraft pose, decoded Rotunda-to-real-wall agreement, exact five-
// part bridge continuity, grounded authored bogie and zero applied child lift.
{
  ...
  source = source.replaceAll(
    ...
    `  expect(Number.isFinite(Number(terminalRuntime.a1ExactRotundaToWallWorldMeters))).toBe(true);
  expect(Math.abs(
    Number(terminalRuntime.a1ExactRotundaToWallWorldMeters) - Number(terminalRuntime.terminal4A1JetwayWallDistance),
...
console.log(
  "Prepared KPHX post-build current-head browser gates: route v11, Cab-derived A1 aircraft pose, decoded Rotunda-to-measured-wall agreement, authored parking heading, grounded zero-lift jetway evidence, and CI-stable bidirectional free-drive motion. No raw (0,6.2) aircraft stop or exact-2.4m compact vestibule requirement is reintroduced.",
);
