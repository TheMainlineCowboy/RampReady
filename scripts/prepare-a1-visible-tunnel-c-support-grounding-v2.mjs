...
const marker = "a1-visible-tunnel-c-support-grounding-runtime-v23-world-box-tunnel-b-visible-supports";
const oldImports = [
...
const newImport = 'import { groundA1TunnelCVisibleSupportHardwareV3 } from "../environment/a1TunnelCVisibleSupportGroundingV10.js";';
const callAnchor = "const finalA1VisibleSupportGrounding = groundA1TunnelCVisibleSupportHardwareV3(THREE, finalA1Model);";
let source = fs.readFileSync(trainerPath, "utf8");
if (!source.includes(callAnchor)) throw new Error(`${trainerPath}: V23 cannot find final A1 support call`);
...
for (const required of [newImport, marker, callAnchor]) if (!source.includes(required)) throw new Error(`${trainerPath}: V23 missing ${required}`);
for (const stale of oldImports) if (source.includes(stale)) throw new Error(`${trainerPath}: stale support import survived V23: ${stale}`);
console.log(`Prepared ${marker}: final A1 support correction selects the six diagnosed Tunnel_B_Jetway_0 visible supports by exact world-space bounds, stretches each from fixed upper mount to rendered KPHX pavement, and does not depend on indexed triangle topology.`);
