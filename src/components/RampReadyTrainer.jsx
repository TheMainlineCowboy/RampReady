...
function buildTug() {
  ...
  group.add(box(2.1, 0.44, 1, 0x801516, 0, 0.63, -2.45));
...
export default function RampReadyTrainer() {
  ...
  const checklist = useMemo(
    () => STAGES.map((label, index) => ({ label, state: index < stageIndex ? "complete" : index === stageIndex ? "active" : "pending" })),
    [stageIndex],
  );
  ...
  const idleThrottle = useCallback(() => {
    driveRef.current.throttle = 0;
    setThrottle(0);
    setTrainerMessage("Power idle. Use brake if you need a faster stop.");
  }, [setTrainerMessage]);

  const updateThrottleFromPointer = useCallback(
    (event) => {
  ...
  const releaseNoseGear = useCallback(() => {
    const sim = simRef.current;
    if (!sim || !sim.connected || stageRef.current !== 5) return;
    sim.connected = false;
    sim.velocity = 0;
    sim.steer = 0;
  ...
  useEffect(() => {
    ...
    const tick = (now) => {
      ...
      const target = new THREE.Vector3(
        ...
        1.2,
        sim.connected ? sim.aircraft.position.z : sim.tug.position.z + 3,
      );
    ...
    <div className="rr-shell">
      ...
      <section className="rr-hud">
        ...
        <div className="rr-hud-actions">
          {[0, 2, 3].includes(stageIndex) && (
            <button className="rr-primary" onClick={advance}>{stageIndex === 0 ? "Ready" : stageIndex === 2 ? "Clearance" : "Brake released"}</button>
          )}
          {stageIndex === 1 && (
            <button className={hud.ready ? "rr-primary" : "rr-primary rr-disabled"} disabled={!hud.ready} onClick={connectNoseGear}>
              {hud.ready ? "Connect nose gear" : `Align ${hud.capture == null ? "--" : hud.capture.toFixed(1)} m`}
            </button>
          )}
          ...
          <button className="rr-secondary" onClick={() => setShowDiagnostics((value) => !value)}>{showDiagnostics ? "Hide diagnostics" : "Diagnostics"}</button>
        </div>
      </section>
      ...
      <div className="rr-steer">
        <button onPointerDown={() => { driveRef.current.steer = 1; }} onPointerUp={() => { driveRef.current.steer = 0; }} onPointerCancel={() => { driveRef.current.steer = 0; }} onPointerLeave={() => { driveRef.current.steer = 0; }}>◀</button>
        <button onPointerDown={() => { driveRef.current.brake = true; }} onPointerUp={() => { driveRef.current.brake = false; }} onPointerCancel={() => { driveRef.current.brake = false; }} onPointerLeave={() => { driveRef.current.brake = false; }}>Brake</button>
        <button onPointerDown={() => { driveRef.current.steer = -1; }} onPointerUp={() => { driveRef.current.steer = 0; }} onPointerCancel={() => { driveRef.current.steer = 0; }} onPointerLeave={() => { driveRef.current.steer = 0; }}>▶</button>
      </div>

      <div className="rr-throttle">
        <button className="rr-direction" onClick={() => { const next = driveRef.current.direction === 1 ? -1 : 1; driveRef.current.direction = next; setDirection(next === 1 ? "FWD" : "REV"); }}>{direction}</button>
        <div
          className="rr-custom-slider"
          role="slider"
          aria-label="Throttle"
          aria-valuemin="0"
          aria-valuemax="100"
        ...
        <button className="rr-idle" onClick={idleThrottle}>Idle</button>
