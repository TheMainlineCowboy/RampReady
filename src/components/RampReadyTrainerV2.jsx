...
function buildTug() {
  ...
  // Long towbarless nose-gear cradle projection. The body remains far in front of the nose gear.
...
export default function RampReadyTrainerV2() {
  ...
  const camRef = useRef({ mode: "chase", yaw: 0.14, pitch: 0.1, distance: 19, height: 5 });
  ...
  useEffect(() => {
    ...
    const tick = (now) => {
      ...
      let msg = message;
      if (stageRef.current === 1) msg = ready ? "Capture distance green. Tap Connect nose gear." : `Capture ${capture.toFixed(1)} m. Approach slowly.`;
      if (stageRef.current === 4 && drive.direction !== -1) msg = "Direction should be REV for pushback.";
      ...
      if (msg !== message && (stageRef.current === 1 || stageRef.current === 4)) setMessage(msg);
  ...
  return (
    <div className="rr-shell">
      ...
      <aside className="rr-dev">{hud.debug}</aside>
