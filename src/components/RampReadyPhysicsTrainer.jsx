...
const STAGES = [
  ...
  "Drive tug clear of the aircraft",
  "Scenario complete",
];
...
function buildTug() {
  ...
  cradleLift.add(box(1.8, 0.1, 0.95, 0x111318, 0, 0.22, 2.75));
  cradleLift.add(box(1.7, 0.12, 0.9, 0x111318, 0, 0.34, CRADLE_Z));
  [-1, 1].forEach((side) => cradleLift.add(box(0.16, 0.56, 0.85, 0xffcc00, side * 0.62, 0.55, CRADLE_Z)));
  group.add(cradleLift);
  [-1, 1].forEach((side) => {
  ...
  return { group, wheels, cradleLift };
}
function connectionMetrics(sim) {
  ...
  const forward = new THREE.Vector3(Math.sin(sim.tug.rotation.y), 0, Math.cos(sim.tug.rotation.y));
  const right = new THREE.Vector3(Math.cos(sim.tug.rotation.y), 0, -Math.sin(sim.tug.rotation.y));
  const heading = Math.abs(Math.atan2(Math.sin(sim.aircraft.rotation.y - sim.tug.rotation.y), Math.cos(sim.aircraft.rotation.y - sim.tug.rotation.y)));
  return {
    cradle,
...
export default function RampReadyPhysicsTrainer() {
  ...
  const [hud, setHud] = useState({ speed: 0, stop: STOP_Z - NOSE_START_Z, capture: 0, ready: false, connected: false, articulation: 0, warning: false, score: 100, phase: CONNECTION_PHASES.APPROACH, progress: 0 });
  ...
  const advance = useCallback(() => {
    ...
    if (stageRef.current === 0) {
      stageRef.current = 1; setStage(1); setMessage("Approach from directly in front, straighten, and stop in the green capture envelope.");
    } else if (stageRef.current === 3 && sim.connection.phase === CONNECTION_PHASES.SECURED) {
      stageRef.current = 4; setStage(4); setMessage("Clearance received. Confirm parking brake release.");
  ...
  const capture = useCallback(() => {
    ...
    if (sim.connection.phase === CONNECTION_PHASES.CAPTURING) {
      stageRef.current = 2; setStage(2); setMessage(sim.connection.reason);
    } else setMessage(sim.connection.reason);
  }, []);
  ...
  useEffect(() => {
    ...
    sun.position.set(18, 28, -14); sun.castShadow = true; scene.add(sun);
    buildGround(scene);
    const camera = new THREE.PerspectiveCamera(58, 1, 0.1, 500);
    const { group: tug, wheels, cradleLift } = buildTug();
    const aircraft = buildCRJ700Aircraft(THREE, mat, cyl);
    aircraft.position.set(0, 0, NOSE_START_Z); aircraft.scale.setScalar(0.82); scene.add(tug, aircraft);
    const sim = { renderer, scene, camera, tug, wheels, cradleLift, aircraft, connection: createConnectionState(), dynamics: createPushbackState(), last: performance.now(), lastHud: 0 };
    simRef.current = sim;
    const resize = () => {
    ...
    const tick = (now) => {
      ...
      sim.last = now;
      const metricsBefore = connectionMetrics(sim);
      const clearDistance = Math.hypot(sim.tug.position.x - sim.aircraft.position.x, sim.tug.position.z - sim.aircraft.position.z);
      sim.connection = stepConnection(sim.connection, { metrics: metricsBefore, speed: sim.dynamics.speed, clearDistance }, dt);
      if (stageRef.current === 2 && sim.connection.phase === CONNECTION_PHASES.SECURED) {
        sim.dynamics = createPushbackState({ tugX: sim.tug.position.x, tugZ: sim.tug.position.z, tugYaw: sim.tug.rotation.y, aircraftX: sim.aircraft.position.x, aircraftZ: sim.aircraft.position.z, aircraftYaw: sim.aircraft.rotation.y });
      ...
      tug.position.set(state.tugX, 0, state.tugZ); tug.rotation.y = state.tugYaw;
      if (connectionHasAircraft(sim.connection)) {
        aircraft.position.set(state.aircraftX, 0, state.aircraftZ); aircraft.rotation.y = state.aircraftYaw;
      }
      ...
      cradleLift.position.y = lift * 0.24;
      ...
      if (towing && state.jackknifeWarning) setMessage("Articulation limit approaching. Reduce power and steer back toward alignment.");
      if (towing && Math.abs(state.speed) > TOW_SPEED_CAUTION) scoreRef.current = Math.max(0, scoreRef.current - 0.02);
      if (towing && Math.abs(state.aircraftX) > CENTERLINE_CAUTION_OFFSET) scoreRef.current = Math.max(0, scoreRef.current - 0.02);
      if (towing && state.aircraftZ >= STOP_Z - 0.5) {
        ...
        sim.dynamics.speed = 0; driveRef.current.throttle = 0; setThrottle(0);
        stageRef.current = 6; setStage(6); setMessage(hard ? "Stopped too hard. Straighten, then lower the cradle." : "Good stop. Straighten, then lower the cradle.");
      }
      const target = connectionHasAircraft(sim.connection) ? aircraft.position : tug.position;
      if (cameraRef.current === "driver") {
        const eye = new THREE.Vector3(-0.45, 1.35, -2.15).applyMatrix4(tug.matrixWorld);
        camera.position.lerp(eye, 0.28); camera.lookAt(new THREE.Vector3(0, 1.1, 12).applyMatrix4(tug.matrixWorld));
      } else if (cameraRef.current === "overhead") {
        camera.position.lerp(new THREE.Vector3(target.x, 34, target.z + 2), 0.16); camera.lookAt(target.x, 0, target.z + 5);
      } else {
        camera.position.lerp(new THREE.Vector3(tug.position.x + 9, 6, tug.position.z - 12), 0.12); camera.lookAt(tug.position.x, 1, tug.position.z + 3);
      }
      if (now - sim.lastHud > 100) {
        sim.lastHud = now;
        setHud({ speed: Math.abs(state.speed), stop: stopRemaining, capture: metrics.distance, ready: sim.connection.phase === CONNECTION_PHASES.ALIGNED, connected: connectionHasAircraft(sim.connection), articulation: Math.abs(THREE.MathUtils.radToDeg(state.articulation)), warning: Math.abs(state.articulation) >= JACKKNIFE_WARNING, score: Math.round(scoreRef.current), phase: sim.connection.phase, progress: sim.connection.progress });
      }
  ...
  return <div className="rr-shell">
    ...
    <aside className="rr-metrics"><span>Speed <b>{(hud.speed * 2.237).toFixed(1)} mph</b></span><span>Stop <b>{Math.max(0, hud.stop).toFixed(1)} m</b></span><span>Articulation <b>{hud.articulation.toFixed(1)}°</b></span><span>Connection <b>{hud.phase}</b></span>{[CONNECTION_PHASES.CAPTURING, CONNECTION_PHASES.LOWERING].includes(hud.phase) && <span>Cycle <b>{Math.round(hud.progress * 100)}%</b></span>}</aside>
