...
function groundOf(root) {
  ...
  throw new Error("A1 V2 support sleeves cannot resolve rendered KPHX pavement");
}
function groundYAt(THREE, ground, x, z, hint) {
  ...
  if (!hit?.point) throw new Error(`A1 V2 support pavement ray miss ${x},${z}`);
  return hit.point.y;
}
...
function collectCandidates(THREE, model, ground) {
  ...
  if(!rotunda||!cab) throw new Error("A1 V2 support sleeves cannot resolve Rotunda/Cab axis");
  const found=[];
  model.traverse((mesh)=>{
  ...
  for(const candidate of found){
    if(selected.some((prior)=>Math.hypot(prior.center.x-candidate.center.x,prior.center.z-candidate.center.z)<0.18)) continue;
    selected.push(candidate); if(selected.length===REQUIRED_COUNT) break;
  }
  if(selected.length!==REQUIRED_COUNT) throw new Error(`A1 V2 support sleeves resolved ${selected.length}/${REQUIRED_COUNT} visible suspended members from ${found.length} candidates`);
  return selected;
}
function material(THREE){return new THREE.MeshStandardMaterial({name:"A1 visible lower support sleeve V2",color:0x24292c,roughness:0.82,metalness:0.3});}

export function addA1VisibleSupportSleevesToPavementV2(THREE, model) {
  if(!model?.isObject3D) throw new Error("A1 V2 support sleeves require final exact A1 model");
  ...
  const report=Object.freeze({authority:AUTHORITY,sleeveCount:group.children.length,maximumExtensionMeters:maxExtension,maximumFinalClearanceMeters:maxClearance,sourceGeometryMutated:false,detectionAuthority:"largest-live-suspended-tunnel-b-c-members-v2",evidence});
