...
// Only collapse effectively coincident disconnected surfaces. The real narrow
// support members sit close together, so a centimetre-scale identity threshold
// must not merge neighbouring rods into one candidate.
...
function groundOf(root) {
  ...
  throw new Error("A1 V6 support sleeves cannot resolve rendered KPHX pavement");
}
function groundYAt(THREE, ground, x, z, hint) {
  ...
  if (!hit?.point) throw new Error(`A1 V6 support pavement ray miss ${x},${z}`);
  return hit.point.y;
}
...
function collectCandidates(THREE, model, ground) {
  ...
  if(!rotunda||!cab) throw new Error("A1 V6 support sleeves cannot resolve Rotunda/Cab axis");
  const found=[];
  model.traverse((mesh)=>{
  ...
  if(selected.length<REQUIRED_COUNT) {
    ...
    throw new Error(`A1 V6 support sleeves resolved ${selected.length} distinct visible suspended members; need at least ${REQUIRED_COUNT} from ${found.length} candidates: ${JSON.stringify(diagnostic)}`);
  }
  return selected;
}
function material(THREE){return new THREE.MeshStandardMaterial({name:"A1 visible lower support sleeve V6",color:0x24292c,roughness:0.82,metalness:0.3});}

export function addA1VisibleSupportSleevesToPavementV2(THREE, model) {
  if(!model?.isObject3D) throw new Error("A1 V6 support sleeves require final exact A1 model");
  ...
  // These sleeves represent telescoping lower support tubes in real-world metres.
  // They must NOT inherit the supplied jetway parent's scale/rotation: doing so
  // rescales their geometry after a correct world-space pavement calculation and
  // visibly leaves daylight beneath them. Attach them to the scene root and place
  // every sleeve from its measured world-space source bottom to KPHX pavement.
  root.add(group); root.updateWorldMatrix?.(true,true);
  ...
  for(let i=0;i<selected.length;i++){
    ...
    if (!(height > 0 && height <= MAX_EXTENSION + TOP_OVERLAP + PAVEMENT_BURY)) throw new Error(`A1 V6 invalid sleeve height ${height}`);
    ...
    const finalClearance=Math.abs(finalBox.min.y-(s.rampY-PAVEMENT_BURY));
    if (!Number.isFinite(finalClearance) || finalClearance > 0.015) throw new Error(`A1 V6 sleeve ${i+1} world-space placement drifted by ${finalClearance} m`);
    maxExtension=Math.max(maxExtension,s.extension); maxClearance=Math.max(maxClearance,finalClearance);
    evidence.push(Object.freeze({sourceMesh:s.mesh.name,triangleCount:s.tris.length,x:s.center.x,z:s.center.z,sourceBottomY:s.box.min.y,rampY:s.rampY,extension:s.extension,finalSleeveBottomY:finalBox.min.y,finalSleeveTopY:finalBox.max.y,alongRatio:s.along,lateralDistance:s.lateral}));
  }
  group.updateWorldMatrix(true,true);
  const report=Object.freeze({authority:AUTHORITY,sleeveCount:group.children.length,maximumExtensionMeters:maxExtension,maximumFinalClearanceMeters:maxClearance,sourceGeometryMutated:false,detectionAuthority:"all-distinct-live-suspended-tunnel-b-c-members-v6-world-space-sleeves",distinctCenterThresholdMeters:MIN_DISTINCT_CENTER_METERS,evidence});
