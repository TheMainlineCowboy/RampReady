...
function groundOf(root) {
  ...
  throw new Error("A1 V5 support sleeves cannot resolve rendered KPHX pavement");
}
function groundYAt(THREE, ground, x, z, hint) {
  ...
  if (!hit?.point) throw new Error(`A1 V5 support pavement ray miss ${x},${z}`);
  return hit.point.y;
}
...
function collectCandidates(THREE, model, ground) {
  ...
  if(!rotunda||!cab) throw new Error("A1 V5 support sleeves cannot resolve Rotunda/Cab axis");
  const found=[];
  model.traverse((mesh)=>{
  ...
  if(selected.length<REQUIRED_COUNT) {
    ...
    throw new Error(`A1 V5 support sleeves resolved ${selected.length} distinct visible suspended members; need at least ${REQUIRED_COUNT} from ${found.length} candidates: ${JSON.stringify(diagnostic)}`);
  }
  return selected;
}
function material(THREE){return new THREE.MeshStandardMaterial({name:"A1 visible lower support sleeve V5",color:0x24292c,roughness:0.82,metalness:0.3});}

export function addA1VisibleSupportSleevesToPavementV2(THREE, model) {
  if(!model?.isObject3D) throw new Error("A1 V5 support sleeves require final exact A1 model");
